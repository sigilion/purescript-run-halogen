...
type HALOGEN state action slots output m
  = FProxy (H.HalogenF state action slots output m)
...
runHalogen ::
  ...
  Run ( halogen :: HALOGEN state action slots output m )
...
handleStateHalogen ::
  ...
  Functor m =>
  State state
    ~> Run ( halogen :: HALOGEN state action slots output m | r )
...
runStateHalogen ::
  ...
  Functor m =>
  Run ( state :: STATE state, halogen :: HALOGEN state action slots output m | r ) a ->
  Run ( halogen :: HALOGEN state action slots output m | r ) a
...
runEffectHalogen ::
  forall state action slots output m r a.
  MonadEffect m =>
  Run ( effect :: EFFECT, halogen :: HALOGEN state action slots output m | r ) a ->
  Run ( halogen :: HALOGEN state action slots output m | r ) a
...
runAffHalogen ::
  forall state action slots output m r a.
  MonadAff m =>
  Run ( aff :: AFF, halogen :: HALOGEN state action slots output m | r ) a ->
  Run ( halogen :: HALOGEN state action slots output m | r ) a
...
handleChildHalogen ::
  ...
  SProxy label ->
  f
    ~> Run ( halogen :: HALOGEN state action slots output m, except :: FAIL | r )
...
runChildHalogen ::
  forall state action output m r label slots f output' _1 r' a.
  Cons label (Slot.Slot f output' Unit) _1 slots =>
  Cons label (FProxy f) ( halogen :: HALOGEN state action slots output m, except :: FAIL | r ) r' =>
  Functor m =>
  IsSymbol label =>
  SProxy label ->
  Run r' a ->
  Run ( halogen :: HALOGEN state action slots output m, except :: FAIL | r ) a
